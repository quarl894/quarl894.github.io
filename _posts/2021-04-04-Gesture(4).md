...
TouchInputMapper에서는 단순히 finger 터치 외에도 stylus, gesture 등을 처리합니다.<br>그 중 이번에는 Gesture event에 대해 알아보려 합니다.</br>

## Gesture
Gesture를 가공하는 가장 중요한 메소드는 preparePointerGestures 입니다.<br><mark>preparePointerGestures만 알면 Gesture가 어떻게 가공되는지 알 수 있습니다.</mark></br>
```java
...
<mark>preparePointerGestures에서 성공적으로 받으면, 동작을 진행하고 아니면 끝낸다.</mark><br>preparePointerGestures에서  cancelPreviousGesture, finishPreviousGesture 또한 동작하는데 cancel일 경우 돌아와서 cancel을 보내고, finish일 경우에만 dispatchMotion으로 값을 보낸다.</br>
...
먼저 Gesture Timeout에 관하여 handling을 한다.<br>이후 velocity update 해줌으로써 좌표계산을 하고 case문으로 각 Gesture에 맞게 가공을 합니다.</br>
